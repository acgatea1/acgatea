// RUN: dataflow-scheduler-opt --reduction-dim-chunking="num-chunks=2" %s | FileCheck %s

// Test: with multiple reduction dims, only the outermost dim is chunked.
//
// Input linalg.generic: iterator_types = ["reduction", "reduction", "parallel"]
// over tensor<2x256x64xf16> → tensor<64xf16>.
//
// num-chunks=2: applied to outermost dim-0 (size=2) → 2 chunks of 1.
//               dim-1 (size=256) is left intact (inner dims skipped).
//
// Single scf.for over dim-0 (bound=2). One ktdf.pipeline per iteration.
//
// is_first = (iv_0 == 0).
//
// Row offset: offset_d0 = iv_0 * 1  (→ memref dim 1)
//
// Transfer indices: [batch, offset_d0, 0, 0], sizes: [1, 1, 256, 64].
...
// CHECK-NEXT:             scf.for %[[VAL_4:.*]] = %[[CONSTANT_4]] to %[[CONSTANT_3]] step %[[CONSTANT_5]] {
// CHECK-NEXT:               %[[CMPI_0:.*]] = arith.cmpi eq, %[[VAL_4]], %[[CONSTANT_4]] : index
// CHECK-NEXT:               ktdf.pipeline {
// CHECK-NEXT:                 %[[PRIVATE_1:.*]]:5 = ktdf.private -> (!ktdf.fifo.slot<"L1LU" -> "SFU", 16384xf16>, !ktdf.fifo.slot<"L1LU" -> "SFU", 64xf16>, !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, !ktdf.token, !ktdf.token) {
...
// CHECK-NEXT:                   ktdf.private_yield %[[FIFO_0]], %[[FIFO_1]], %[[FIFO_2]], %[[CREATE_TOKEN_2]], %[[CREATE_TOKEN_3]] : !ktdf.fifo.slot<"L1LU" -> "SFU", 16384xf16>, !ktdf.fifo.slot<"L1LU" -> "SFU", 64xf16>, !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, !ktdf.token, !ktdf.token
// CHECK-NEXT:                 }
...
// CHECK-NEXT:                   %[[SUBI_0:.*]] = arith.subi %[[VAL_3]], %[[CONSTANT_6]] : index
// CHECK-NEXT:                   %[[DIVSI_0:.*]] = arith.divsi %[[SUBI_0]], %[[CONSTANT_7]] : index
// CHECK-NEXT:                   ktdf.data_transfer from %[[VAL_2]]#0{{\[}}%[[DIVSI_0]], %[[VAL_4]], %[[CONSTANT_6]], %[[CONSTANT_6]]] size [1, 1, 256, 64] to %[[VAL_5]]#0 size [16384] : memref<1x2x256x64xf16, "L1">, !ktdf.fifo.slot<"L1LU" -> "SFU", 16384xf16>
...
// CHECK-NEXT:                   }
// CHECK-NEXT:                   %[[GENERIC_0:.*]] = linalg.generic {indexing_maps = [#[[$ATTR_0]], #[[$ATTR_1]]], iterator_types = ["reduction", "reduction", "parallel"]} ins(%[[READ_FROM_FIFO_0]] : tensor<1x256x64xf16>) outs(%[[IF_0]] : tensor<64xf16>) {
...
// CHECK-NEXT:                   ktdf.write_to_fifo %[[GENERIC_0]], %[[VAL_6]]#2 : tensor<64xf16>, <"SFU" -> "L1SU", 64xf16>
...
// CHECK-NEXT:                   %[[SUBI_1:.*]] = arith.subi %[[VAL_3]], %[[CONSTANT_8]] : index
// CHECK-NEXT:                   %[[DIVSI_1:.*]] = arith.divsi %[[SUBI_1]], %[[CONSTANT_9]] : index
// CHECK-NEXT:                   ktdf.data_transfer from %[[VAL_9]]#2 size [64] to %[[VAL_2]]#1{{\[}}%[[DIVSI_1]], %[[CONSTANT_8]]] size [1, 64] : !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, memref<1x64xf16, "L1">
...
// CHECK-NEXT:           scf.for %[[VAL_11:.*]] = %[[CONSTANT_0]] to %[[CONSTANT_1]] step %[[CONSTANT_1]] {
// CHECK-NEXT:             ktdf.data_transfer from %[[VAL_10]]#1{{\[}}%[[VAL_11]], 0] size [1, 64] to %[[CAST_1]]{{\[}}%[[CONSTANT_0]]] size [64] : memref<1x64xf16, "L1">, memref<64xf16, strided<[1], offset: ?>, "DDR">
// CHECK-NEXT:           } {loop_type = #ktdf.loop_type<parallel_loop>}
