// RUN: dataflow-scheduler-opt --reduction-opacification %s | FileCheck %s
...
// CHECK-NEXT:         %[[PRIVATE_0:.*]]:4 = ktdf.private -> (memref<256x2x1x64xf16, #ktdp.memory_space<ct_local>>, memref<1x64xf16, "SFU_REG">, !ktdf.token, !ktdf.token) {
...
// CHECK-NEXT:           ktdf.private_yield %[[ALLOC_0]], %[[ALLOC_1]], %[[CREATE_TOKEN_0]], %[[CREATE_TOKEN_1]] : memref<256x2x1x64xf16, #ktdp.memory_space<ct_local>>, memref<1x64xf16, "SFU_REG">, !ktdf.token, !ktdf.token
// CHECK-NEXT:         }
...
// CHECK-NEXT:                   }
// CHECK-NEXT:                 } {loop_type = #ktdf.loop_type<reduction_loop>}
// CHECK-NEXT:                 ktdf.opaque "simd_reduction" {dataflow_scheduler.register_names = ["t0_0", "t0_0"], func_name = "simdreduction", parameter_dictionary = {unroll = "1"}}
...
// CHECK-NEXT:                 scf.for %[[VAL_11:.*]] = %[[CONSTANT_4]] to %[[CONSTANT_10]] step %[[CONSTANT_5]] {
// CHECK-NEXT:                   %[[SUBI_2:.*]] = arith.subi %[[VAL_3]], %[[CONSTANT_0]] : index
// CHECK-NEXT:                   %[[DIVSI_2:.*]] = arith.divsi %[[SUBI_2]], %[[CONSTANT_1]] : index
// CHECK-NEXT:                   %[[CMPI_0:.*]] = arith.cmpi eq, %[[VAL_11]], %[[CONSTANT_6]] : index
// CHECK-NEXT:                   scf.if %[[CMPI_0]] {
// CHECK-NEXT:                     ktdf.data_transfer from %[[VAL_10]]#1 size [1, 64] to %[[VAL_2]]#1[0, 0] size [1, 64] : !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, memref<1x64xf16, "SFU_REG">
// CHECK-NEXT:                   }
// CHECK-NEXT:                 } {loop_type = #ktdf.loop_type<reduction_loop>}
...
// CHECK-NEXT:             ktdf.data_transfer from %[[VAL_12]]#1[0, 0] size [1, 64] to %[[CAST_1]]{{\[}}%[[VAL_13]], %[[CONSTANT_0]] * 64] size [1, 64] : memref<1x64xf16, "SFU_REG">, memref<256x64xf16, strided<[64, 1], offset: ?>, #ktdp.memory_space<global>>
// CHECK-NEXT:           } {loop_type = #ktdf.loop_type<parallel_loop>}
...
// Tests that the inner-dimension reduction linalg.generic (ins=%alloc,
// outs=%subview) is replaced by ktdf.opaque "simd_reduction" (ins=%alloc,
// outs=%alloc) and that the now-dead memref.subview is erased.
// The outer-dim loop, linalg.fill, and all other ops are left unchanged.
...
module {
  ...
  module @local_schedule_0 {
    func.func @local_schedule_0() attributes {grid = [1]} {
      ...
      ktdf.pipeline {
        %2:4 = ktdf.private -> (memref<256x2x1x64xf16, #ktdp.memory_space<ct_local>>, memref<1x64xf16, "SFU_REG">, !ktdf.token, !ktdf.token) {
          %alloc = memref.alloc() : memref<256x2x1x64xf16, #ktdp.memory_space<ct_local>>
          %alloc_3 = memref.alloc() : memref<1x64xf16, "SFU_REG">
          %3 = ktdf.create_token : !ktdf.token
          %4 = ktdf.create_token : !ktdf.token
          ktdf.private_yield %alloc, %alloc_3, %3, %4 : memref<256x2x1x64xf16, #ktdp.memory_space<ct_local>>, memref<1x64xf16, "SFU_REG">, !ktdf.token, !ktdf.token
        }
          ...
          scf.for %arg0 = %c0 to %c256 step %c1 {
            ...
            ktdf.pipeline {
              ...
              ktdf.stage depends_in(%3#2) depends_out(%3#3) {
                ...
                linalg.fill ins(%cst : f16) outs(%alloc : memref<1x64xf16, "SFU_REG">)
                %c2 = arith.constant 2 : index
                scf.for %arg1 = %c0_3 to %c2 step %c1_4 {
                  %4 = ktdf.read_from_fifo %3#0 : <"L1LU" -> "SFU", 64xf16> -> memref<1x1x64xf16>
                  linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["reduction", "parallel", "parallel"]} ins(%4 : memref<1x1x64xf16>) outs(%alloc : memref<1x64xf16, "SFU_REG">) {
                  ^bb0(%in: f16, %out: f16):
                    %5 = arith.addf %in, %out : f16
                    linalg.yield %5 : f16
                  }
                } {loop_type = #ktdf.loop_type<reduction_loop>}
                %subview = memref.subview %alloc[0, 0] [1, 1] [1, 1] : memref<1x64xf16, "SFU_REG"> to memref<1x1xf16, strided<[64, 1]>, "SFU_REG">
                linalg.generic {indexing_maps = [#map2, #map3], iterator_types = ["parallel", "reduction", "parallel"]} ins(%alloc : memref<1x64xf16, "SFU_REG">) outs(%subview : memref<1x1xf16, strided<[64, 1]>, "SFU_REG">) {
                ^bb0(%in: f16, %out: f16):
                  %4 = arith.addf %in, %out : f16
                  linalg.yield %4 : f16
                }
                ktdf.write_to_fifo %alloc, %3#1 : memref<1x64xf16, "SFU_REG">, <"SFU" -> "L1SU", 64xf16>
              ...
              ktdf.stage depends_in(%3#3) depends_out(none) {
                ...
                scf.for %arg1 = %c0_3 to %c2 step %c1_4 {
                  ...
                  scf.if %6 {
                    ktdf.data_transfer from %3#1 size [1, 64] to %2#1[0, 0] size [1, 64] : !ktdf.fifo.slot<"SFU" -> "L1SU", 64xf16>, memref<1x64xf16, "SFU_REG">
                  }
                } {loop_type = #ktdf.loop_type<reduction_loop>}
        ...
        ktdf.stage depends_in(%2#3) depends_out(none) {
          scf.for %arg0 = %c0 to %c256 step %c1 {
            %3 = arith.subi %arg0, %c0 : index
            %4 = arith.divsi %3, %c1 : index
            ktdf.data_transfer from %2#1[0, 0] size [1, 64] to %cast_2[%arg0, %c0 * 64] size [1, 64] : memref<1x64xf16, "SFU_REG">, memref<256x64xf16, strided<[64, 1], offset: ?>, #ktdp.memory_space<global>>
          } {loop_type = #ktdf.loop_type<parallel_loop>}
